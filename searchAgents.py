...
def foodHeuristic(state, problem):
    ...
    if len(foods) == 0:
        return 0
    mhtDistsToFoods = map(manhattanDist(position), foods)
    farthest_food = foods[mhtDistsToFoods.index(max(mhtDistsToFoods))]
    actualDist = mazeDistance(position, farthest_food, problem.startingGameState)
    mhtDist = max(mhtDistsToFoods)
    return 0.5 * (actualDist + mhtDist)
